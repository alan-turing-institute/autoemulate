...
@pytest.fixture()
def model_registry():
    return {"gp": GaussianProcessSk(), "rf": RandomForest(), "rbf": RBF()}
...
def test_get_models(model_registry):
    ...
    assert all([isinstance(model, BaseEstimator) for model in models.values()])


def test_get_models_subset(model_registry):
    models = _get_models(model_registry, model_subset=["gp", "rf"])
    assert len(models) == 2
    model_names = model_registry.keys()
    assert all([model_name in model_registry for model_name in model_names])
    assert all([isinstance(model, BaseEstimator) for model in models.values()])
...
def test_wrap_models_in_pipeline_no_scaler(model_registry):
    ...
    assert all([model.steps[0][0] != "scaler" for model in models.values()])
    print(models)
...
def test_wrap_models_in_pipeline_no_scaler_dim_reducer(model_registry):
    ...
    assert all([isinstance(model, Pipeline) for model in models.values()])
    assert all([model.steps[0][0] == "dim_reducer" for model in models.values()])
...
def test_wrap_models_in_pipeline_scaler_dim_reducer(model_registry):
    ...
    # assert that pipeline does have a scaler as first step
    assert all([model.steps[0][0] == "scaler" for model in models.values()])
    assert all([model.steps[1][0] == "dim_reducer" for model in models.values()])
