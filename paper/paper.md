...
We further extended the capabilities of AutoEmulate to expose a range of downstream tasks in a single package. The comprehensive nature of the tool distinguishes it from more narrowly focused packages. For example, SBI [@sbi] or BayesFlow [@bayesflow] can train emulators on simulated data and perform Bayesian calibration while Ax [@ax] or scikit-activeml [@skactiveml] support active learning, but none of these cover as wide a range of emulation capabilities.
