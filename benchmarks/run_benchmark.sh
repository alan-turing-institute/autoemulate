...
for simulator in Epidemic FlowProblem Projectile ProjectileMultioutput; do
  for n_iter_pair in "10 100" "150 50" "200 20"; do
    for n_splits in 5 2; do  
      ...
      pueue add "python benchmarks/benchmark.py --simulators \"$simulator\" --n_splits_list \"$n_splits\" --n_iter_list \"$n_iter1\" --n_iter_list \"$n_iter2\" --output_file \"${outpath}/benchmark_results_${simulator}_n_splits_${n_splits}_n_iter_${n_iter1}_${n_iter2}.csv\""
    done
  done
done
