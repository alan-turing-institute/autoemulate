...
def _validate_inputs(cv_results, model_name):
    ...
    """
    if not cv_results:
        msg = "Run .compare() first."
        raise ValueError(msg)

    if model_name and model_name not in cv_results:
        raise ValueError(
...
def _check_multioutput(y, output_index):
    ...
    if y.ndim > 1:
        if (output_index > y.shape[1] - 1) | (output_index < 0):
            msg = f"Output index {output_index} is out of range. "
            msg += f"The index should be between 0 and {y.shape[1] - 1}."
            raise ValueError(msg)
        print(
...
def _plot_single_fold(  # noqa: PLR0913
    cv_results,
    X,
    y,
    model_name,
    ...
    plot : str, optional
        ...
        “standard” draws the observed values (y-axis)
        vs. the predicted values (x-axis) (default).
        “residual” draws the residuals,
        i.e. difference between observed and predicted values,
    ...
    if plot_type == "Xy":
        if input_index >= X.shape[1]:
            ...
            msg += f"The index should be between 0 and {X.shape[1] - 1}."
            raise ValueError(msg)
        ...
        display = PredictionErrorDisplay.from_predictions(  # noqa: F841
            y_true=y_test,
            y_pred=y_test_pred,
            kind=plot_type,
            ax=ax,
...
def _plot_best_fold_per_model(  # noqa: PLR0913
    cv_results,
    X,
    y,
    n_cols=3,
...
def _plot_model_folds(  # noqa: PLR0913
    cv_results,
    X,
    y,
    model_name,
...
def _plot_cv(  # noqa: PLR0913
    ...
    model_name : (str, optional)
        The name of the model to plot. If None, the best (largest R^2)
        fold for each model will be plotted.
    n_cols : int, optional
        The maximum number of columns in the plot. Default is 3.
    plot : str, optional
        The type of plot to draw:
        “standard” draws the observed values (y-axis)
        vs. the predicted values (x-axis) (default).
        “residual” draws the residuals, i.e. difference between observed and
        predicted values, (y-axis) vs. the predicted values (x-axis).
    figsize : tuple, optional
        Overrides the default figure size.
    output_index : int, optional
...
def _plot_model(  # noqa: PLR0912, PLR0913
    model,
    X,
    y,
    style="Xy",
    ...
    plot : str, optional
        ...
        "standard" draws the observed values (y-axis)
        vs. the predicted values (x-axis) (default).
        "residual" draws the residuals, i.e. difference between observed
    ...
    input_index : int or list of int, optional
        The index(es) of the input feature(s) to plot for "Xy" plots.
...
def _display_figure(fig):
    ...
    # Are we in Jupyter?
    try:
        is_jupyter = get_ipython().__class__.__name__ == "ZMQInteractiveShell"  # type: ignore PGH003
    except NameError:
        is_jupyter = False
