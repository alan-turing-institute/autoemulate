...
class MCMCVisualizationDashboard:
    ...
    def _create_ui(self):
        ...
        self.param_y = widgets.Dropdown(
            options=self.param_names,
            value=(self.param_names[1] if len(self.param_names) > 1 
        ...
        self.trace_controls = widgets.VBox([
            self.burnin
        ])
    ...
    def _plot_corner(self, figsize):
        ...
        if len(selected_params) < 2:
            ...
            plt.text(0.5, 0.5, 'Corner plot requires at least 2 parameters',
                    ha='center', va='center', fontsize=14)
            plt.axis('off')
            return
        
        selected_data = self._get_selected_data(selected_params, apply_burnin=True)
        samples_array = np.column_stack([selected_data[param] for param in selected_params])
    ...
    def _plot_traces(self, figsize):
        ...
        for i, param in enumerate(selected_params):
            ...
            ax.axhline(post_burnin_mean, color='red', linestyle='--', alpha=0.7, label='Mean')
            ax.legend()
    ...
    def _plot_posteriors(self, figsize):
        ...
        for i, param in enumerate(selected_params):
            ...
            # Histogram
            ax.hist(samples, bins=self.n_bins.value, alpha=0.7, 
                   density=True, color='skyblue', edgecolor='black', label='Posterior')
            ...
            if self.show_prior.value and hasattr(self.calibrator, 'reduced_bounds'):
                if param in self.calibrator.reduced_bounds:
                    ...
                    ax.axhline(prior_height, color='orange', linestyle='--', 
            ...
            ax.axvline(mean_val, color='red', linewidth=2, label=f'Mean: {mean_val:.3f}')
    ...
    def _plot_correlations(self, figsize):
        ...
        if len(selected_data) < 2:
            plt.figure(figsize=figsize)
            plt.text(0.5, 0.5, 'Correlation plot requires at least 2 parameters',
                    ha='center', va='center', fontsize=14)
            plt.axis('off')
            return
    ...
    def _plot_pairwise(self, figsize):
        ...
        if len(selected_params) < 2:
            plt.figure(figsize=figsize)
            plt.text(0.5, 0.5, 'Pairwise plot requires at least 2 parameters',
                    ha='center', va='center', fontsize=14)
            plt.axis('off')
            return
        ...
        if self.sample_size.value:
            ...
            if first_param_len > self.sample_size.value:
                indices = np.random.choice(first_param_len, size=self.sample_size.value, replace=False)
                selected_data = {param: samples[indices] for param, samples in selected_data.items()}
    ...
    def _plot_convergence(self, figsize):
        ...
        for i, param in enumerate(selected_params):
            ...
            ax1.axhline(np.mean(samples), color='red', linestyle='--', label='Final Mean')
            ...
            autocorr = [np.corrcoef(samples[:-lag], samples[lag:])[0, 1] if lag > 0 
                       else 1.0 for lag in lags]
            
            ax2.plot(lags, autocorr, 'o-', markersize=3)
            ax2.axhline(0, color='red', linestyle='--', alpha=0.5)
            ax2.axhline(0.1, color='orange', linestyle=':', alpha=0.5, label='10% threshold')
    ...
    def _plot_vs_prior(self, figsize):
        ...
        for i, param in enumerate(selected_params):
            ...
            if self.show_kde.value:
                ...
                ax.hist(samples, bins=self.n_bins.value, alpha=0.7, 
                       density=True, color='skyblue', label='Posterior')
            
            # Prior if available
            if hasattr(self.calibrator, 'reduced_bounds') and param in self.calibrator.reduced_bounds:
                bounds = self.calibrator.reduced_bounds[param]
                prior_height = 1 / (bounds[1] - bounds[0])
                ax.axhline(prior_height, color='red', linestyle='--', 
                          linewidth=3, label='Prior (Uniform)')
