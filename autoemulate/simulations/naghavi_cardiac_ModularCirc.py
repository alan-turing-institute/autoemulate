...
class NaghaviSimulator:
    def __init__(self, parameters_range: Dict[str, Tuple[float, float]],
        ...
        self._output_names = []  # Will be populated with expanded stat names after first simulation
                ...
                if (not attr_name.startswith('_') and 
                    ...
                    try:
                        ...
                        if hasattr(attr, 'values'):
                            ...
                            if not self._output_variables or full_name in self._output_variables:
                                values = np.array(attr.values)
                                
                                # Get both stats and stat names
                                stats, stat_names = self._calculate_output_stats(values, full_name)
