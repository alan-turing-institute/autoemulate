...
def plot_reactiondiffuison(x, y, n=64, L=20, title='Reaction-Diffusion Simulation', cmap='viridis'):
    ...
    n_samples = x.shape[0]
    y_plot = y.reshape(n_samples, n, n)
    
    # Plot maximum 3 instances
    n_sim = min(3, n_samples)
    x_size = int(n_sim*5) 

    plt.figure(figsize=(x_size, 4))
    for i in range(n_sim):
        plt.subplot(1, n_sim, i + 1)
        plt.pcolor(X_grid, Y_grid, y_plot[i, :, :], cmap=cmap)
        #plt.xlabel('x', fontsize=12)
        #plt.ylabel('y', fontsize=12, rotation=0)
        plt.title(f'beta= {x[i][0]:.2f}, d = {x[i][1]:.2f}', fontsize=12)
        ax = plt.gca()
        #ax.set_yticklabels([])
        #ax.set_xticklabels([])
    plt.suptitle(title + '\n', fontsize=16)
