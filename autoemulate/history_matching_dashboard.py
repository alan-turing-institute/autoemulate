...
class HistoryMatchingDashboard:
    ...
    def _create_ui(self):
        ...
        self.param_y = widgets.Dropdown(
            options=self.param_names,
            value=self.param_names[1] if len(self.param_names) > 1 else self.param_names[0],
        ...
        self.param_z = widgets.Dropdown(
            options=self.param_names,
            value=self.param_names[2] if len(self.param_names) > 2 else self.param_names[0],
        ...
        self.nroy_filter = widgets.Checkbox(
            value=False,
            description='Show only NROY points',
            disabled=False
        )
        ...
        controls_bottom = widgets.HBox([self.sample_selector, self.wave_selector, self.nroy_filter])
    ...
    def _update_plot(self, _):
        ...
        with self.output:
            ...
            filtered_df['NROY'] = np.max(filtered_scores, axis=1) <= threshold if len(filtered_scores.shape) > 1 else filtered_scores <= threshold
            
            # Check if we have data to plot
            if len(filtered_df) == 0:
                plt.figure(figsize=(10, 6))
                plt.text(0.5, 0.5, 'No data to display with current filters', 
    ...
    def _plot_parameter_vs_implausibility(self, df, impl_scores):
        ...
        # Create scatter plot
        plt.scatter(df[param], min_impl, c=max_impl, cmap='viridis_r', 
                   alpha=0.7, s=50, label='Min Implausibility')
    ...
    def _plot_pairwise_parameters(self, df, impl_scores):
        ...
        # Create scatter plot
        sc = plt.scatter(df[param_x], df[param_y], c=max_impl, cmap='viridis_r',
                      alpha=0.7, s=50)
        
        # Highlight NROY points with an outline
        nroy_points = df[df['NROY'] == True]
        if not nroy_points.empty:
            plt.scatter(nroy_points[param_x], nroy_points[param_y], s=80,
                      facecolors='none', edgecolors='g', linewidths=2,
                      label='NROY Points')
            
            # Add NROY region boundaries
            plt.axvline(x=nroy_points[param_x].min(), color='g', linestyle='--', alpha=0.5)
            plt.axvline(x=nroy_points[param_x].max(), color='g', linestyle='--', alpha=0.5)
            plt.axhline(y=nroy_points[param_y].min(), color='g', linestyle='--', alpha=0.5)
            plt.axhline(y=nroy_points[param_y].max(), color='g', linestyle='--', alpha=0.5)
    ...
    def _plot_implausibility_distribution(self, impl_scores):
        ...
        plt.title(f'Implausibility Distribution\n'
    ...
    def _plot_nroy_parameter_ranges(self, df):
        ...
        if nroy_df.empty:
            plt.figure(figsize=(12, 6))
            plt.text(0.5, 0.5, 'No NROY points found with current threshold', 
                    ha='center', va='center', fontsize=14)
            plt.axis('off')
            plt.tight_layout()
            plt.show()
        ...
        for i, param in enumerate(param_names):
            ...
            reduction = (1 - nroy_range / original_range) * 100 if original_range > 0 else 0
            
            # Create range plot
            plt.plot([original_min, original_max], [0, 0], 'b-', linewidth=10, alpha=0.3, label='Original Range')
            plt.plot([nroy_min, nroy_max], [0, 0], 'g-', linewidth=10, alpha=0.7, label='NROY Range')
            
            # Add points for all NROY values
            plt.plot(nroy_df[param], np.zeros_like(nroy_df[param]), 'g|', markersize=15, alpha=0.5)
    ...
    def _plot_implausibility_radar(self, df, impl_scores):
        ...
        if sample_idx < len(impl_scores):
            ...
            # Plot threshold
            plt.plot(angles, [threshold]*len(angles), 'r--', linewidth=1, label='Threshold')
            ...
            param_str = '\n'.join([f"{key}={value:.3g}" for key, value in sample_params.items()])
            
            plt.title(f"Implausibility Radar for Sample {sample_idx}\n{param_str}", fontsize=12)
            plt.legend(loc='upper right')
            ...
            plt.figure(figsize=(10, 6))
            plt.text(0.5, 0.5, 'Sample index out of range', 
                   ha='center', va='center', fontsize=14)
            plt.axis('off')
            plt.tight_layout()
            plt.show()
