...
def plot_implausibility_distribution(impl_scores, threshold=3.0, figsize=(10, 6)):
    ...
    plt.title(
        f"Implausibility Score Distribution\n"
...
def plot_parameter_vs_implausibility(
    samples,
    impl_scores,
    param_names,
    threshold=3.0,
...
def plot_pairwise_parameters(
    samples, impl_scores, param_names=None, threshold=3.0, max_pairs=6, figsize=(15, 12)
):
    """
        ...
        for j in range(i + 1, n_params):
            if pair_count < n_pairs:
                ...
                if df["NROY"].sum() > 0:
                    nroy_df = df[df["NROY"]]
                    ax.axvline(
                        x=nroy_df[param_names[i]].min(),
                        color="g",
                        linestyle="--",
                        alpha=0.5,
                    )
                    ax.axvline(
                        x=nroy_df[param_names[i]].max(),
                        color="g",
                        linestyle="--",
                        alpha=0.5,
                    )
                    ax.axhline(
                        y=nroy_df[param_names[j]].min(),
                        color="g",
                        linestyle="--",
                        alpha=0.5,
                    )
                    ax.axhline(
                        y=nroy_df[param_names[j]].max(),
                        color="g",
                        linestyle="--",
                        alpha=0.5,
                    )
...
def plot_wave_convergence(wave_results, param_names=None, figsize=(14, 10)):
    ...
    if len(all_wave_samples) > 0:
        if (
            isinstance(all_wave_samples[0], list)
            and len(all_wave_samples[0]) > 0
            and isinstance(all_wave_samples[0][0], dict)
        ):
            wave0_df = pd.DataFrame(all_wave_samples[0])
        else:
            # Assume it's already a DataFrame
            wave0_df = (
                all_wave_samples[0].copy()
                if hasattr(all_wave_samples[0], "copy")
                else pd.DataFrame(all_wave_samples[0])
            )
    else:
        # No waves to plot
        plt.figure(figsize=figsize)
        plt.text(
            0.5,
            0.5,
            "No wave data available to plot",
            ha="center",
            va="center",
            fontsize=14,
        )
    ...
    for i, param in enumerate(param_names):
        ...
        for wave_idx, wave_samples in enumerate(all_wave_samples):
            ...
            if (
                isinstance(wave_samples, list)
                and len(wave_samples) > 0
                and isinstance(wave_samples[0], dict)
            ):
                ...
                wave_df = (
                    wave_samples.copy()
                    if hasattr(wave_samples, "copy")
                    else pd.DataFrame(wave_samples)
                )
...
def plot_pca_nroy(
    samples, impl_scores, threshold=3.0, n_components=2, figsize=(12, 10)
):
    """
...
def plot_implausibility_radar(
    ...
    for i, idx in enumerate(sample_indices):
        ...
        param_title = "\n".join(
            [", ".join(param_strs[i : i + 3]) for i in range(0, len(param_strs), 3)]
        )
...
def visualize_history_matching_results(
    samples_list,
    impl_scores_list,
    param_names,
    output_names,
    ...
    try:
        ...
        plt_wave = plot_wave_convergence(
            wave_results, param_names, figsize=(figsize_base[0], figsize_base[1] * 0.8)
        )
        ...
        plt_param_impl.savefig(
            "param_vs_implausibility.png", dpi=300, bbox_inches="tight"
        )
        ...
        if len(param_names) >= 2:
            ...
            plt_pairwise.savefig(
                "pairwise_parameters.png", dpi=300, bbox_inches="tight"
            )
            plt_pairwise.close()
        ...
        for i, impl_scores in enumerate(impl_scores_list[: min(max_waves, n_waves)]):
            plt_dist = plot_implausibility_distribution(
                impl_scores,
                threshold,
                figsize=(figsize_base[0] * 0.8, figsize_base[1] * 0.5),
            )
            plt_dist.savefig(
                f"implausibility_dist_wave{i+1}.png", dpi=300, bbox_inches="tight"
            )
            plt_dist.close()
        ...
        if output_names and len(output_names) > 0:
            try:
                ...
                plt_radar.savefig(
                    "implausibility_radar.png", dpi=300, bbox_inches="tight"
                )
                plt_radar.close()
            except Exception as e:
        ...
        print(
            f"4. implausibility_dist_wave1.png to implausibility_dist_wave{min(n_waves, max_waves)}.png - Implausibility distributions"
        )
        if len(param_names) > 2:
            print("5. pca_nroy.png - PCA visualization of NROY space")
        if output_names and len(output_names) > 0:
            print(
                "6. implausibility_radar.png - Radar plots of implausibility for best samples"
            )
        if len(param_names) >= 3:
...
def plot_history_matching_results(hm, all_samples, all_impl_scores, n_waves=3):
    ...
    if (
        isinstance(all_samples, list)
        and len(all_samples) > 0
        and isinstance(all_samples[0], dict)
    ):
        ...
        samples_df = (
            all_samples.copy()
            if hasattr(all_samples, "copy")
            else pd.DataFrame(all_samples)
        )
