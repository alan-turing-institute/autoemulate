...
if __name__ == "__main__":
    ...
    if not os.path.exists(f"simulator_results_{n_parameters}_{N_samples}_sa_fixed.csv"):
        ...
        if save:
            # Save the results to a CSV file
            results_df.to_csv(
                f"simulator_results_{n_parameters}_{N_samples}_sa_fixed.csv", index=False
            )
            inputs_df.to_csv(f"simulator_inputs_{n_parameters}_{N_samples}_sa_fixed.csv", index=False)

    else:
        # Read the results from the CSV file
        results_df = pd.read_csv(f"simulator_results_{n_parameters}_{N_samples}_sa_fixed.csv")
        inputs_df = pd.read_csv(f"simulator_inputs_{n_parameters}_{N_samples}_sa_fixed.csv")
