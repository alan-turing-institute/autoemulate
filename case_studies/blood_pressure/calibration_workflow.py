...
def plot_implausibility(df, wave, top_parameters_sa, ref_val, output_path="implausibility_plot"):
    """
    ...
    for i, xi in enumerate(top_parameters_sa):
        for j, yj in enumerate(top_parameters_sa):
            if j < i:  # lower triangle only
                ...
                ax.scatter(
                    ref_val[j], ref_val[i],
                    color="white", s=60, edgecolor="black", marker="X", zorder=5,
                    label="True value" if (i == len(top_parameters_sa) - 1 and j == 0) else None
                )
...
if __name__ == '__main__':
    ...
    if not os.path.exists(f'simulator_results_{N_samples}_sa_fixed.csv'):
        ...
        if save:
            # Save the results to a CSV file
            results_df.to_csv(f'simulator_results_{N_samples}_sa_fixed.csv', index=False)
